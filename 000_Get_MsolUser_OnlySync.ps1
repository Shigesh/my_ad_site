...
# 列名出力
"UserPrincipalName" + "`t" + "ImmutableID" + "`t" + "LastDirSyncTime"+ "`t" + "objectID"+ "`t" + "msDS_cloudExtensionAttribute1"+ "`t" + "msDS_cloudExtensionAttribute2" > $ret
...
[String]$UserPrincipalName = [String]$MsolUser[$i].UserPrincipalName
     ...
     if ( -Not ( ([String]$UserPrincipalName).Contains("#EXT#") -or ([String]$UserPrincipalName).Substring(0,5) -eq "Sync_" )){    
        ...
        $UserPrincipalName + "`t" + $MsolUser[$i].ImmutableID + "`t" + $MsolUser[$i].LastDirSyncTime + "`t" + $MsolUser[$i].objectID + "`t" + $ex_attri_1 + "`t" + $ex_attri_2 >> $ret
    }
